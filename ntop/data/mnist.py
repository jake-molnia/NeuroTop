...
def get_mnist_loaders(batch_size: int = 64, train_size: Optional[int] = None,
...
def create_simple_mnist_model(hidden_dims: list = [256, 128], dropout: float = 0.2) -> nn.Module:
    return simple_mlp(input_dim=784, hidden_dims=hidden_dims, output_dim=10, dropout=dropout)

def get_quick_mnist(batch_size: int = 64, n_samples: int = 1000) -> Tuple[DataLoader, DataLoader]:
...
class MNISTConfig:
    ...
    def create_model(self) -> nn.Module:
        return create_simple_mnist_model(self.hidden_dims, self.dropout)
    
    def create_loaders(self) -> Tuple[DataLoader, DataLoader]:
        return get_mnist_loaders(self.batch_size, self.train_size, self.test_size)